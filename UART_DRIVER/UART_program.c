...
{
	UDR = TxData;// the default at the begining is that the regs is empty
	
	// Busy Wait on Data Register empty flag
	while(0 == GET_BIT(UCSRA,UDRE));//to ensure that the flag that indicate that the data register is empty is raised
}
